...
class TestAWSSynthSampleCSV:
    ...
    def test_postgres_compatibility(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            converter = FocusConverter()
            converter.load_provider_conversion_configs()
            converter.load_data(str(test_csv_path), DataFormats.CSV)
            ...
            output_csv_files = [f for f in all_csv_files if "aws_synthetic_cur" not in f.name]
            ...
            # Test Postgres table compatibility - all required columns from focus_line_items table
            postgres_columns = [
                ...
                "BillingAccountId",   # payerAccountId (nullable)
                "ResourceId",         # resourceId (nullable)
                "EffectiveCost",      # effectiveCost
                "ConsumedQuantity",   # usageQuantity (nullable)
                "BillingCurrency",    # currencyCode
                "RegionId",           # region (nullable)
                "AvailabilityZone",   # availabilityZone (nullable)
                "ServiceCategory",    # productFamily (nullable)
                "ConsumedUnit"        # pricingUnit (nullable)
            ]
            ...
            # Test data types for key fields (CSV export converts to strings, so check for valid datetime strings)
            # For CSV output, datetime fields are strings in various formats
            if "BillingPeriodStart" in result_df.columns:
                # Check if it's a valid datetime string
                sample_value = str(result_df["BillingPeriodStart"][0])
                assert sample_value is not None and sample_value != "null", "BillingPeriodStart should not be null"
                # Check if it looks like a datetime (has date pattern)
                assert ("T" in sample_value or " " in sample_value) and "-" in sample_value, f"BillingPeriodStart should be datetime string, got: {sample_value}"
                
            if "ChargePeriodStart" in result_df.columns:
                sample_value = str(result_df["ChargePeriodStart"][0])
                assert sample_value is not None and sample_value != "null", "ChargePeriodStart should not be null"
                assert ("T" in sample_value or " " in sample_value) and "-" in sample_value, f"ChargePeriodStart should be datetime string, got: {sample_value}"
                
            if "EffectiveCost" in result_df.columns:
                # Numeric fields should be readable as float
                assert result_df.schema["EffectiveCost"] in [pl.Float64, pl.Float32, pl.Int64, pl.Utf8], "EffectiveCost should be numeric or string"
                
            if "ConsumedQuantity" in result_df.columns:
                assert result_df.schema["ConsumedQuantity"] in [pl.Float64, pl.Float32, pl.Int64, pl.Utf8], "ConsumedQuantity should be numeric or string"
            
            # Test required fields are not null (based on Postgres constraints)
            required_non_null = ["Provider", "BillingPeriodStart", "ChargePeriodStart", 
                                "ServiceName", "SubAccountId", "EffectiveCost", "BillingCurrency"]
            ...
            # Test EffectiveCost >= 0 constraint
            negative_costs = result_df.select((pl.col("EffectiveCost") < 0).sum()).item()
            assert negative_costs == 0, f"Found {negative_costs} negative EffectiveCost values"
            
            # Test BillingCurrency is 3 characters (CHAR(3) constraint)
            currency_lengths = result_df.select(pl.col("BillingCurrency").str.len_chars()).to_series()
            invalid_lengths = (currency_lengths != 3).sum()
            assert invalid_lengths == 0, f"Found {invalid_lengths} invalid currency code lengths"
